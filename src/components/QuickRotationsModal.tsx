...
export const QuickRotationsModal: React.FC<QuickRotationsModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className="saloon-glass relative w-full max-w-md rounded-3xl border-2 border-amber-900/30 bg-[#0e121a]/95 p-6 text-cream shadow-2xl"
      ...
      >
        ...
        {/* Rotations List */}
        <div className="mt-4 max-h-[380px] overflow-y-auto space-y-2.5 pr-1">
          {PLAYLISTS.map((playlist) => {
            ...
            return (
              <div
                ...
                className={`saloon-glass group flex items-center justify-between rounded-2xl p-3 sm:p-3.5 transition-all cursor-pointer hover:border-cream/30 ${
              ...
              >
                ...
                <button
                  ...
                  className={`saloon-chip text-xs shrink-0 py-1.5 px-3 ${isActive ? 'active' : ''}`}
                >
                  {isActive && isPlaying ? (
                    <>
